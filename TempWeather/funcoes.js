    ...
    minhasBuscas.forEach(busca => {
      ...
      html += `
        <div class="buscas" data-id="${busca._id}">
          ...
          <button class="btn-apagar-busca">❌</button>
        </div>
      `;
    });
...
function getCorMinMax(tempmin, tempmax) {
  if (tempmin&&tempmax <= 0) return "aqua";
  if (tempmin&&tempmax <= 5) return "royalblue";
  if (tempmin&&tempmax <= 10) return "blue";
  if (tempmin&&tempmax <= 15) return "#1a7534";
  if (tempmin&&tempmax <= 20) return "yellow";
  if (tempmin&&tempmax <= 25) return "orange";
  if (tempmin&&tempmax <= 30) return "orangered";
  return 'red';
}